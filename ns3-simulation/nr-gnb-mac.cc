...
NrGnbMac::DoReportBufferStatus(NrMacSapProvider::ReportBufferStatusParameters params)
{
    ...
    std::cout << Simulator::Now().GetSeconds() <<  " RNTI: " << params.rnti<<" Buffer Tx Level: " << params.txQueueSize << " Buffer TxHol Level: " << params.txQueueHolDelay  << " Buffer Rx Level: " << params.retxQueueSize << " Buffer RxHol Level: " << params.retxQueueHolDelay << " PDU: " << params.statusPduSize<< std::endl;
}
