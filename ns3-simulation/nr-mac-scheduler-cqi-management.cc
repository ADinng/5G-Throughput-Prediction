                                             ...
                                             int8_t maxDlMcs) const
{
    ...
    //add new NS_LOG_INFO about cqi...
    //calculate achievable rate
    uint32_t tbSize = GetAmcDl()->CalculateTbSize(ueInfo->m_dlMcs, 1, 1); // 1 indicates rank，1 indicates PRB quantity
    double achievableRate = (tbSize * 8.0) / 0.001; // convert to bps

    NS_LOG_INFO("Time: " << Simulator::Now().GetSeconds() 
                << " RNTI = " << ueInfo->m_rnti 
                << " CellID = " << GetCellId()
                << " WB CQI = " << static_cast<uint32_t>(ueInfo->m_dlCqi.m_wbCqi)
