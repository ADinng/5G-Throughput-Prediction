...
NrUePhy::NrUePhy()
{
    ...
    Simulator::Schedule(m_wbCqiPeriodicity, &NrUePhy::GeneratePeriodicCqiReport, this);
}
    ...
    static TypeId tid =
        TypeId("ns3::NrUePhy")
            ...
            .AddAttribute("DownlinkCqiPeriodicity",
                          ...
                          MakeTimeAccessor(&NrUePhy::SetDownlinkCqiPeriodicity),
                          MakeTimeChecker())
...
NrUePhy::DoSendControlMessage(Ptr<NrControlMessage> msg)
{
    ...
    EnqueueCtrlMessage(msg); //messagetype 2

    // std::cout << Simulator::Now().GetSeconds() << " RNTI: " << m_rnti
    //           << ", Sending control message type: " << msg->GetMessageType() << std::endl; //messagetype 2
...
NrUePhy::PhyCtrlMessagesReceived(const Ptr<NrControlMessage>& msg)
{
    ...
    if (msg->GetMessageType() == NrControlMessage::DL_DCI) //messagetype = 1
    {
    ...
    else if (msg->GetMessageType() == NrControlMessage::UL_DCI) //messagetype = 0
    {
    ...
    else if (msg->GetMessageType() == NrControlMessage::MIB) //messagetype = 3
    {
    ...
    else if (msg->GetMessageType() == NrControlMessage::SIB1) //messagetype = 4
    {
    ...
    else if (msg->GetMessageType() == NrControlMessage::RAR) //messagetype = 6
    {
...
NrUePhy::UlCtrl(const std::shared_ptr<DciInfoElementTdma>& dci)
{
    ...
    for (const auto& msg : m_ctrlMsgs)
    {
        ...
        if (msg->GetMessageType() == NrControlMessage::DL_HARQ) //messagetype = 8
        {
...
void
NrUePhy::SetDownlinkCqiPeriodicity(Time cqiPeriodicity)
{
    NS_LOG_FUNCTION(this << cqiPeriodicity);
    m_wbCqiPeriodicity = cqiPeriodicity;
}


void
NrUePhy::GeneratePeriodicCqiReport()
{
    if (m_ulConfigured && (m_rnti > 0))
    {
        if (Simulator::Now() > m_wbCqiLast + m_wbCqiPeriodicity)
        {
            if (m_cachedCtrlSinr)
            {
                ...
                if (msg)
                {
                    ...
                    NS_LOG_INFO("Periodic CQI report sent at time " << Simulator::Now().GetMilliSeconds() << " ms");
                    // std::cout << Simulator::Now().GetSeconds() << " "<< " Periodic CQI report sent at time " << std::endl;
                }
                m_wbCqiLast = Simulator::Now();
            }
            else
            {
                NS_LOG_WARN("No cached SINR available for periodic CQI report");
            }
        }
    }
    Simulator::Schedule(m_wbCqiPeriodicity, &NrUePhy::GeneratePeriodicCqiReport, this);

    NS_LOG_INFO("CQI Report decision point: now=" << Simulator::Now().GetSeconds()
              << ", m_wbCqiLast=" << m_wbCqiLast.GetSeconds()
              << ", cachedSinr=" << (m_cachedCtrlSinr ? "yes" : "no"));
}
